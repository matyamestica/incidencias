...
function newUserProblem(req, res){
    ...
    Problem.findOne(problemId, newUser, (err, newUserProblem) => {

        newUserProblem.users.push({users: newUser}); 

        if(err){
            res.status(500).send({message: 'Error al agregar nuevo usuario'});
        }else{
            if (!newUserProblem){
                res.status(404).send({message: 'No se ha podido actualizar el problema'});
            }else{
                res.status(200).send({problem: newUserProblem});
            }
        }
    });
